...
#            self.sighting_sets_base_url = f"{self.base_url}{sighting_id}/sets?fields=" # to append fields
    ...
    def fetch_data(self, *, sighting_id=None, query_id=None, sighting_field_list=None, query_field_list=None,  fetch_links=False, fetch_sets=False):
        ...
        if not sighting_id:
            if fetch_links or fetch_sets: 
                 print("❌ Warning: Links and Sets can only be got when you assign an sighting_id")
        
        if not query_id: 
             print("❌ Warning: Links and Sets can only be fetched for a sighting, not query")
        ...
        if query_id:
            ...
            if response.status_code == 200:
                self.sighting_json = response.json()
                self.query_id_list = [item['id'] for item in res_json['data']]
            else:
                print("Failed to fetch data")
                return None
